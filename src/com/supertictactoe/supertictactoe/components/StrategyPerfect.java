...
public class StrategyPerfect implements Strategy {
  ...
  public Move nextMove(Game game) {
    ...
    Move nextMove = continueCombo(game, valid_board);
    if (!game.cellIsValid(nextMove.getBoard(), nextMove.getCell())) {
      System.out.println("So far this strategy's logic has been unhandled! " +
			 "Time to see what the next move should be.");
    }
    return nextMove;
  }
  ...
  protected Move continueCombo(Game game, int valid_board) {
    return new Move(valid_board,
		game.cellIsValid(valid_board, boardWithFreeMiddle) ?
		boardWithFreeMiddle :
		boardOppositeBWFM,
		Side.NIL);
  }
